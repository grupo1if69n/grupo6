 ...
 * @author hideki
 */
public class FecharContaFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (session.getAttribute("uNome") == null) {
            ...
            if (uc.getConta().getCGerente().equals(uNome) && uc.getConta().getCAberto() && uc.getConta().getCValor() > 0.0) {
                chain.doFilter(request, response);
            } else if (uc.getConta().getCGerente().equals(uNome) && uc.getConta().getCAberto() && uc.getConta().getCValor() == 0.0) {
                 ((HttpServletResponse) response).sendRedirect(((HttpServletRequest) request).getContextPath() + "/faces/gerenciar_conta.xhtml");
            } else {
                ((HttpServletResponse) response).sendRedirect(((HttpServletRequest) request).getContextPath() + "/faces/sistema.xhtml");
            }
        }
